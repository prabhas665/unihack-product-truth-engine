...
class _FlakyLLMClient(LLMClient):
    """Offline client that times out the first N calls, then succeeds."""
        ...
        *,
        ...
        if self.calls <= self._failures:
            raise LLMTimeoutError(
                f"{self.provider}: timed out (attempt {self.calls})"
            )
        return '{"ok": true}'


class TestRetry:
    def test_timeout_is_retried_then_succeeds(self, monkeypatch):
        monkeypatch.setattr(settings, "llm_retry_attempts", 2)
        monkeypatch.setattr(settings, "llm_retry_backoff_seconds", 0.0)
        client = _FlakyLLMClient(failures=1)
        assert client.complete(CompletionRequest(user_prompt="hi")) == '{"ok": true}'
        assert client.calls == 2

    def test_retries_are_exhausted(self, monkeypatch):
        monkeypatch.setattr(settings, "llm_retry_attempts", 2)
        monkeypatch.setattr(settings, "llm_retry_backoff_seconds", 0.0)
        client = _FlakyLLMClient(failures=5)
        with pytest.raises(LLMTimeoutError):
            client.complete(CompletionRequest(user_prompt="hi"))
        assert client.calls == 2

    def test_retry_disabled_runs_once(self, monkeypatch):
    ...
    def test_non_timeout_error_is_not_retried(self, monkeypatch):
        monkeypatch.setattr(settings, "llm_retry_attempts", 3)
        monkeypatch.setattr(settings, "llm_retry_backoff_seconds", 0.0)
        client = FakeLLMClient(error=LLMInvalidResponseError("nope"))
        with pytest.raises(LLMInvalidResponseError):
            client.complete(CompletionRequest(user_prompt="hi"))
