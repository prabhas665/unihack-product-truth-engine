"""OpenRouter LLM provider adapter (Step 7A).

Implements the OpenRouter chat/completions API (OpenAI-compatible) behind the
existing LLMClient abstraction: subclasses implement ONLY ``_complete()``;
JSON parsing, markdown-fence tolerance, schema validation, and typed error
mapping all live in the base class (app/llm/base.py) - nothing is duplicated
here.
...
class OpenRouterClient(LLMClient):
        ...
        *,
        ...
        if not key:
            raise LLMConfigurationError(
                "LLM_API_KEY is not set: the openrouter provider requires it "
    ...
    @classmethod
    def from_settings(cls, settings: object | None = None) -> "OpenRouterClient":
    ...
    def _complete(
        ...
        *,
        ...
        """One chat/completions call; returns choices[0].message.content."""
        messages: list[dict] = []
        if system_prompt:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"OpenRouterClient(provider={self.provider!r}, model={self._model!r}, "
